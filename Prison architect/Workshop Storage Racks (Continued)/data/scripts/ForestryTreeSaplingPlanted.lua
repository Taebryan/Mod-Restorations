...
function toggleMaxAgePlusClicked()
	if maxAge < 24 then
		...
		this.Tooltip = { "tooltip_TreeAge",age,"X",this.MaxAge,"Y",maxAge*3,"Z" }
	else
		maxAge = 1
		Set(this,"MaxAge",maxAge)
		this.SetInterfaceCaption("CaptionMaxAge", "tooltip_Caption_MaxAge",this.MaxAge,"X")
...
function LoadTree()
	...
	if next(nearbyTree) then
		...
		newTree = Object.Spawn("ForestryTreeYoung",this.Pos.x,this.Pos.y)
...
--		this.SubType = subToTreeType[Get("TreeType")]
		...
		age = Get("Age") or -0.25
		Set(this,"Age",age)
		this.Tooltip = { "tooltip_TreeAge",this.Age,"X",this.MaxAge,"Y",maxAge*3,"Z" }
	...
	if timeTot >= timePerUpdate then
		...
		if age >= maxAge then
			if not this.DummySpawned then
				...
				newDummyTree.Age = 2879				-- we only need to show the growing effect
				Set(this,"DummySpawned",true)
				timePerUpdate = 1
			else
