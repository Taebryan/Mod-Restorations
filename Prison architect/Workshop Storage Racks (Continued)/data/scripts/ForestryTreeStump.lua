...
function toggleMaxAgePlusClicked()
	if maxAge < 24 then
		...
		this.Tooltip = { "tooltip_TreeAge",age+maxAge+maxAge+maxAge,"X",this.MaxAge,"Y",maxAge*3,"Z" }
	else
		maxAge = 1
		Set(this,"MaxAge",maxAge)
		this.SetInterfaceCaption("CaptionMaxAge", "tooltip_Caption_MaxAge",this.MaxAge,"X")
...
function Update(timePassed)
	...
	if timeTot >= timePerUpdate then
		...
		Set(this,"Age",age)
		this.Tooltip = { "tooltip_TreeAge",age+maxAge+maxAge+maxAge,"X",this.MaxAge,"Y",maxAge*3,"Z" }
		if age >= maxAge+1 then	-- in case the job messed up, spawn a fresh sapling
			Object.CancelJob(this,"PlantForestryTree")
			newTree = Object.Spawn("ForestryTreeSaplingPlanted",this.Pos.x,this.Pos.y)
			...
			Object.CreateJob(this,"PlantForestryTree")
		end
	end
end
...
function JobComplete_PlantForestryTree()
	newTree = Object.Spawn("ForestryTreeSaplingPlanted",this.Pos.x,this.Pos.y)
	Set(newTree,"TimeWarp",this.TimeWarp)
	Set(newTree,"MaxAge",this.MaxAge)
	Set(newTree,"TreeType",this.TreeType)
